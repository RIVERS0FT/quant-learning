...
- AkShare stock_individual_fund_flow：近约 100 个交易日个股资金流
...
class Config:
    ...
    distance_w_corr: float = 0.50
    distance_w_sector: float = 0.20
    distance_w_factor: float = 0.30
    # 历史回测默认不使用估值距离，避免用“当前估值”污染历史截面。
    distance_w_valuation: float = 0.00
...
def load_data(
    ...
    for symbol, meta in universe.items():
        ...
        try:
            df = fetch_price(symbol, start_date, end_date).merge(
                fetch_flow(symbol), on="date", how="left"
            )
            df["name"] = meta["name"]
...
def add_features(data: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    def per_symbol(g: pd.DataFrame) -> pd.DataFrame:
        ...
        g["volatility"] = g["return_1d"].rolling(cfg.volatility_window).std() * np.sqrt(252)
        g["avg_amount"] = g["amount"].rolling(cfg.liquidity_window).mean()
        g["liquidity"] = np.log1p(g["avg_amount"].clip(lower=0))
        g["flow_ratio"] = g["main_net_flow"] / g["amount"].replace(0, np.nan)
        g["flow_strength"] = g["flow_ratio"].rolling(cfg.flow_window).mean()
        return g

    parts = [per_symbol(g) for _, g in data.groupby("symbol", sort=False)]
    ...
    df["outflow_budget"] = (-df["main_net_flow"]).clip(lower=0).fillna(0.0)
    return df
...
def build_distance(
    ...
    trailing = features[
        (features["date"] <= date) & features["symbol"].isin(symbols)
    ...
    factor_cols = ["momentum_z", "volatility_z", "liquidity_z", "flow_strength_z"]
    ...
    # valuation_frame 必须是真实的“该日期历史估值”，不能把当前 PE/PB 用于过去回测。
    if valuation_frame is not None and cfg.distance_w_valuation > 0:
        val = valuation_frame.reindex(symbols).apply(zscore, axis=0).fillna(0.0)
        components.append((cfg.distance_w_valuation, pairwise_euclidean(val)))
...
def gravity_flows(day: pd.DataFrame, distance: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    symbols = day.index.tolist()
    source = day["outflow_budget"].reindex(symbols).fillna(0).clip(lower=0).to_numpy(float)
    attraction = day["attractiveness"].reindex(symbols).fillna(1).clip(lower=1e-8).to_numpy(float)

    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        raw = (
            np.power(source, cfg.alpha)[:, None]
            * np.power(attraction, cfg.beta)[None, :]
            / np.power(distance.to_numpy(float), cfg.gamma)
        )
    raw[~np.isfinite(raw)] = 0.0
    np.fill_diagonal(raw, 0.0)

    row_sum = raw.sum(axis=1, keepdims=True)
    weights = np.divide(raw, row_sum, out=np.zeros_like(raw), where=row_sum > 0)
...
def build_snapshot(
    ...
    required = [
        "main_net_flow", "avg_amount", "momentum_z", "volatility_z",
        "liquidity_z", "flow_strength_z", "attractiveness",
    ]
    day = features[features["date"] == date].dropna(subset=required)
    ...
    result["predicted_price_pressure"] = cfg.impact_lambda * result["migration_pressure"]
...
def rank_ic(x: pd.Series, y: pd.Series) -> float:
    ...
    if len(pair) < 3:
        return np.nan
    xr, yr = pair.iloc[:, 0].rank(), pair.iloc[:, 1].rank()
    if xr.nunique() < 2 or yr.nunique() < 2:
        return np.nan
    return float(xr.corr(yr))


def backtest(features: pd.DataFrame, cfg: Config) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    valid = features.dropna(
        subset=["main_net_flow", "flow_strength", "momentum", "volatility", "avg_amount", "next_return"]
    )
    counts = valid.groupby("date")["symbol"].nunique()
    dates = counts[counts >= cfg.min_cross_section].index.sort_values()
    ...
    for date in dates:
        ...
        ic_rows.append({"date": date, "rank_ic": rank_ic(day["migration_pressure"], day["next_return"]), "n": len(day)})

        ranked = day.sort_values("migration_pressure", ascending=False)
        ...
        portfolio_rows.append(
            {
                ...
                "equal_weight_return": ranked["next_return"].mean(),
            }
        )
    ...
    if not portfolio.empty:
        for col in ["top_n_return", "bottom_n_return", "long_short_return", "equal_weight_return"]:
            portfolio[f"{col}_nav"] = (1 + portfolio[col].fillna(0)).cumprod()
...
def save_results(
    ...
    latest.reset_index().to_csv(out / "stock_capital_migration_snapshot.csv", index=False, encoding="utf-8-sig")
    edges.to_csv(out / "stock_capital_migration_edges.csv", index=False, encoding="utf-8-sig")
    predictions.to_csv(out / "stock_capital_migration_backtest.csv", index=False, encoding="utf-8-sig")
    ic_df.to_csv(out / "stock_capital_migration_ic.csv", index=False, encoding="utf-8-sig")

    if not no_plot and not portfolio.empty:
        ax = portfolio.plot(
            x="date",
            y=["top_n_return_nav", "equal_weight_return_nav"],
            figsize=(12, 6),
            grid=True,
            title="Stock Capital Migration: Top-N vs Equal Weight",
        )
        ax.set_ylabel("NAV")
        ax.legend(["Migration Top-N", "Equal Weight"])
        plt.tight_layout()
        plt.savefig(out / "stock_capital_migration_nav.png", dpi=150)
        plt.close()


def print_summary(date: pd.Timestamp, latest: pd.DataFrame, edges: pd.DataFrame, ic_df: pd.DataFrame, portfolio: pd.DataFrame, top_n: int) -> None:
    print(f"\n========== 股票资本迁移模型：{date.date()} ==========")
    cols = ["name", "sector", "main_net_flow", "attractiveness", "net_migration", "migration_pressure"]
    view = latest[cols].copy()
    view["main_net_flow"] /= 1e8
    view["net_migration"] /= 1e8
    print("\n迁移压力排名：")
    print(view.rename(columns={"main_net_flow": "主力净流入(亿)", "net_migration": "净迁移(亿)"}).to_string())

    if not edges.empty:
        top_edges = edges.head(12).copy()
        top_edges["inferred_flow"] /= 1e8
        print("\n最大推断迁移路径：")
        print(top_edges[["source_name", "target_name", "inferred_flow"]].rename(columns={"inferred_flow": "推断迁移(亿)"}).to_string(index=False))

    if not ic_df.empty:
        print(f"\n平均 Rank IC：{ic_df['rank_ic'].mean():.4f}")
        print(f"Rank IC > 0 占比：{(ic_df['rank_ic'] > 0).mean():.2%}")
    if not portfolio.empty:
        top_nav = portfolio["top_n_return_nav"].iloc[-1]
        eq_nav = portfolio["equal_weight_return_nav"].iloc[-1]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--start-date", default=(pd.Timestamp.today() - pd.DateOffset(years=2)).strftime("%Y%m%d"))
...
def main() -> None:
    ...
    raw = load_data(DEFAULT_UNIVERSE, args.start_date, args.end_date)
    features = add_features(raw, cfg)

    valid = features.dropna(subset=["main_net_flow", "flow_strength", "momentum", "volatility", "avg_amount"])
    counts = valid.groupby("date")["symbol"].nunique()
    valid_dates = counts[counts >= cfg.min_cross_section].index.sort_values()
    if len(valid_dates) == 0:
        raise RuntimeError("没有足够完整的资金流截面")

    latest_date = valid_dates[-1]
    latest, flow = build_snapshot(latest_date, features, cfg)
    day = features[features["date"] == latest_date].drop_duplicates("symbol").set_index("symbol")
    edges = flow_edges(flow, day)
    predictions, ic_df, portfolio = backtest(features, cfg)

    save_results(latest, edges, predictions, ic_df, portfolio, args.no_plot)
    print_summary(latest_date, latest, edges, ic_df, portfolio, cfg.top_n)
